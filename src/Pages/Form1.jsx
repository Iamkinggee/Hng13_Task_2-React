...
//   const handleSubmit = (e, type) => {
//     e.preventDefault();
//     const userEmailInput = e.target.email.value.trim();
//     const userPassword = e.target.password.value.trim();

//     const emailRegex = /^[^\s@]+@[^\s@]+\.com$/;

//     if (!emailRegex.test(userEmailInput)) {
//       setMessage("❌ Please enter a valid email ending with '.com'");
//       setMsgType("error");
//       setEmailError(true);
//       setShake(true);
//       setTimeout(() => setShake(false), 400);
...
//     if (userEmailInput && userPassword) {
//       if (!localStorage.getItem("user")) {
//         localStorage.setItem(
//           "user",
//           JSON.stringify([{ email: userEmailInput, password: userPassword }])
//         );
//         setMessage("✅ Account created successfully!");
//         setMsgType("success");
//         setUserEmail(userEmailInput);
//         setShowPopup(true); // ✅ show popup after signup
//       } else {
//         for (let val of data) {
//           setEmail(val.email);
//           if (val.email.includes(userEmailInput)) {
//             if (type === "signUp") {
//               setMessage("⚠️ User already exists!");
//               setMsgType("error");
//               setSignIn(true);
...
//         if (type === "signUp" && email !== userEmailInput) {
//           localStorage.setItem(
//             "user",
//             JSON.stringify([...data, { email: userEmailInput, password: userPassword }])
//           );
//           setMessage("✅ Account created successfully!");
//           setMsgType("success");
//           setUserEmail(userEmailInput);
...
//         <div className="grid md:grid-cols-2 items-center gap-4 max-md:gap-8 max-w-6xl max-md:max-w-lg w-full p-4 rounded-md">
//           {/* ===== FORM SIDE ===== */}
//           <div className="md:max-w-md w-full px-4 py-6 bg-white rounded-2xl shadow-lg">
//             <form
//               onSubmit={(e) => handleSubmit(e, signin ? "signIn" : "signUp")}
//               noValidate
//             >
...
//               {/* Toggle */}
//               {!signin ? (
//                 <p className="text-[15px] mt-6 text-slate-600 text-center">
//                   Already have an account?
//                   <a
//                     href="#"
//                     className="text-green-600 font-medium hover:underline ml-1"
...
//           {/* ===== IMAGE SIDE ===== */}
//           <div className="shadow-lg w-[90%] flex items-center bg-green-600 rounded-[50%] p-8">
//             <img
//               src={manage1}
//               alt="Illustration"
//               className="w-full aspect-[15/15] object-contain"
...
//               {signin ? "Login Successful 🎉" : "Account Created 🎉"}
//             </h2>
//             <p className="text-gray-700 mb-5">Continue to your dashboard?</p>
//             <div className="flex justify-center gap-4">
//               <button
//                 onClick={() => navigate("/home", { state: userEmail })}
...
function Form1() {
  ...
  const continueToDashboard = () => {
    setShowPopup(false);
    navigate("/home", { state: localStorage.getItem("loggedInEmail") });
  };
  ...
  return (
    <div className="relative h-screen">
      ...
      {showPopup && (
        <div
          className="fixed inset-0 flex justify-center items-center bg-black/30 backdrop-blur-sm z-50 animate-fadeIn"
          onClick={() => setShowPopup(false)}
        >
          <div
            className="bg-white rounded-xl p-6 shadow-xl text-center w-80 transform animate-popup"
